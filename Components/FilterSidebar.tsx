...
const FilterSidebar = ({
  ...
  function getSameProblemOtherContestId(
    ...
    if (sameRoundContestIds) {
      if (sameRoundContestIds.length === 2) {
        ...
        } else if (sameRoundContestIds.length === 1 
                && sameRoundContestIds[0] !== problemContestID) {
  ...
  const handleFilter = async (isNewFilter: boolean) => {
    ...
    if (isNewFilter) {
      ...
      const calculatedPageNumber:number = Math.max(1, Math.ceil(newProblems.length / problemsPerPage));
      if (pageNumber > calculatedPageNumber) {
        setPageNumber(calculatedPageNumber);
      }
    } else {
      setFilteredProblems([...filteredProblems, ...newProblems]);
      const calculatedPageNumber:number = Math.max(1, Math.ceil((filteredProblems.length + newProblems.length) / problemsPerPage));
      if (pageNumber > calculatedPageNumber) {
        setPageNumber(calculatedPageNumber);
      }
    }
    ...
    for (const tag in tagState) {
      tagButtons.push(
        <button
          key={tag}
          type="button"
          className={`text-xs ${tagState[tag] ? activeTopicStyleTag : topicStyle}`}
          onClick={() => {
            handleTagToggle(tag);
          }}
        >
